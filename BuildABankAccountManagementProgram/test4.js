...
let displayBooks = (arrBook) => {
  let titleAuthorPages = "";
  arrBook.forEach((book) => {
    titleAuthorPages += `${book.title} by ${book.author}, ${book.pages} pages\n`;
  });
  return titleAuthorPages;
};

let getBooksByAuthor = (arrBook, author) => {
  let bookByAuthor = [];
  arrBook.forEach((book) => {
    if (author === book.author) {
      bookByAuthor.push(book.title);
    }
  });
  return bookByAuthor;
};
